<template>
  <div class="hello" ref="wrapper">
    <div  class="content">
    <ul>
      <li v-for="(v,i) in yinyue" :key="i" @click="bofang(v)">
        <i v-show="i+1 > 3">{{i+1}}</i>
        <span :id="i+1 == 1?'one':i+1==2?'er':''" class="iconfont icon-jiangbei span" v-show="i+1<=3"></span>
        <div>

          <h2>{{v.data.songname}}</h2>
          <p>
            <span v-for="(value,index) in v.data.singer" :key="index">{{value.name}}</span>
          </p>
        </div>
      </li>
    </ul>
    ...
    bofang(v){
      ...
      '%22%5D%2C%22songtype%22%3A%5B0%5D%2C%22uin%22%3A%220%22%2C%22loginflag%22%3A1%2C%22platform%22%3A%2220%22%7D%7D%2C%22comm%22%3A%7B%22uin%22%3A0%2C%22format%22%3A%22json%22%2C%22ct%22%3A20%2C%22cv%22%3A0%7D%7D').then((res)=>{
        than.songurl = res.data.req.data.sip[0]+res.data.req_0.data.midurlinfo[0].purl
          than.$axios.get('/ed/lyric/fcgi-bin/fcg_query_lyric_new.fcg?-=MusicJsonCallback_lrc&pcachetime=1547778937953&songmid='+v.data.songmid+'&g_tk=1127320055&loginUin=2764590450&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0').then((src)=>{
              than.lyric = src.data.lyric
              than.songname = v.data.songname
              than.songername = v.data.singer[0].name
