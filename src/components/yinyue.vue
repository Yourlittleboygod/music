<template>
  <div class="hello" ref="wrapper">
    <div  class="content">
    <ul>
      <li v-for="(v,i) in zhi" :key="i"  @click="qiehuan(v)">
        <h2>{{v.musicData.songname}}</h2>
        <p>
          <span v-for="(value,index) in v.musicData.singer" :key="index">{{value.name}}</span>
        </p>
      </li>
    </ul>
    </div>
    ...
    qiehuan(v){
      ...
      '%22%5D%2C%22songtype%22%3A%5B0%5D%2C%22uin%22%3A%220%22%2C%22loginflag%22%3A1%2C%22platform%22%3A%2220%22%7D%7D%2C%22comm%22%3A%7B%22uin%22%3A0%2C%22format%22%3A%22json%22%2C%22ct%22%3A20%2C%22cv%22%3A0%7D%7D').then((res)=>{
        than.songurl = res.data.req.data.sip[0]+res.data.req_0.data.midurlinfo[0].purl
          than.$axios.get('/ed/lyric/fcgi-bin/fcg_query_lyric_new.fcg?-=MusicJsonCallback_lrc&pcachetime=1547778937953&songmid='+v.musicData.songmid+'&g_tk=1127320055&loginUin=2764590450&hostUin=0&format=json&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq.json&needNewCode=0').then((src)=>{
              than.lyric = src.data.lyric
              console.log(v.musicData)
              than.songname = v.musicData.songname
              than.songername = v.musicData.singer[0].name
